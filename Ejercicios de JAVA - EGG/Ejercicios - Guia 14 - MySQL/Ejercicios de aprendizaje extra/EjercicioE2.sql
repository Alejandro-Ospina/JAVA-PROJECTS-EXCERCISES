...
Select pais as "País", ciudad as "Ciudades de España", telefono as "Número de teléfono de las ciudades de España" from oficina where 
pais = "España";

##3. Devuelve un listado con el nombre, apellidos y email de los empleados cuyo jefe tiene un código de jefe igual a 7.
select nombre as "Nombre empleado", concat(apellido1,' ',apellido2) as "Apellidos del empleado", email from empleado where codigo_jefe=7;

##4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa.
select nombre as "Nombre jefe", concat(apellido1,' ',apellido2) as "Apellidos jefe", email, puesto from empleado where puesto like "%Director%" or "%Subdirector%";

##5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que no sean representantes de ventas.
select nombre as "Nombre empleado", concat(apellido1,' ',apellido2) as "Apellidos del empleado", puesto from empleado where puesto not in ("Representante Ventas");

##6. Devuelve un listado con el nombre de los todos los clientes españoles.
Select nombre_cliente as "Nombre de los clientes españoles" from cliente where pais = "Spain";

##7. Devuelve un listado con los distintos estados por los que puede pasar un pedido.
select estado as "Lista de estados de un pedido" from pedido group by estado;
...
select codigo_cliente as "Código de clientes que realizaron pagos en el 2008 con la función year" from pago where year(fecha_pago) = 2008 group by codigo_cliente;

#### Utilizando la función DATE_FORMAT de MySQL.
Select codigo_cliente as "Código de clientes que realizaron pagos en el 2008 con la función date format" from pago where date_format(fecha_pago, '%Y') = 2008 group by codigo_cliente;

#### Sin utilizar ninguna de las funciones anteriores.
Select codigo_cliente as "Código de clientes que realizaron pagos en el 2008 sin usar las funciones previas" from pago where fecha_pago like "%2008%" group by codigo_cliente;

##9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de 
##entrega de los pedidos que no han sido entregados a tiempo.
select codigo_pedido as "Código del pedido", codigo_cliente as "Código del cliente", fecha_esperada, fecha_entrega from pedido where fecha_entrega > fecha_esperada;
...
select codigo_pedido as "Código del pedido", codigo_cliente as "Código del cliente", fecha_esperada, fecha_entrega from pedido where adddate(fecha_entrega, 2) <= fecha_esperada;

## Utilizando la función DATEDIFF de MySQL.
select codigo_pedido as "Código del pedido", codigo_cliente as "Código del cliente", fecha_esperada, fecha_entrega from pedido where datediff(fecha_esperada, fecha_entrega) >= 2;

##11. Devuelve un listado de todos los pedidos que fueron rechazados en 2009.
select * from pedido where estado = "Rechazado" and year(fecha_pedido) = 2009;

##12. Devuelve un listado de todos los pedidos que han sido entregados en el mes de enero de cualquier año.
select * from pedido where estado = "Entregado" and date_format(fecha_entrega, '%m') = 1;

##13. Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordene el resultado de mayor a menor.
select * from pago where forma_pago = "Paypal" and year(fecha_pago) = 2008 order by total asc;
